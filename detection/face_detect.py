...
class MTCNN:
    def __init__(self, model_path):
        ...
        # 获取P模型
        self.pnet = torch.jit.load(os.path.join(model_path, 'PNet.pth'),map_location='cpu')
        self.pnet.to(self.device)
        self.softmax_p = torch.nn.Softmax(dim=0)
        self.pnet.eval()

        # 获取R模型
        self.rnet = torch.jit.load(os.path.join(model_path, 'RNet.pth'),map_location='cpu')
        self.rnet.to(self.device)
        self.softmax_r = torch.nn.Softmax(dim=-1)
        self.rnet.eval()

        # 获取O模型
        self.onet = torch.jit.load(os.path.join(model_path, 'ONet.pth'),map_location='cpu')
